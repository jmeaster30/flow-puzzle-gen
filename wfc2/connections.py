...
def getPossibilities(x: int, y: int, boardWidth: int, boardHeight: int) -> List[int]:
  if x == 0:
    if y == 0:
      return [1, 4, 5]
...
def generateBoard(boardWidth: int, boardHeight: int) -> List[List[Node]]:
  ...
  for index in range(boardWidth * boardHeight):
    x = index % boardWidth
    y = index // boardWidth
    result.append(getPossibilities(x, y, boardWidth, boardHeight))
